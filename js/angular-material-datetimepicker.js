(function () {
    ...
    var template = '<md-dialog class="dtp" layout="column" style="width: 300px;">'
        ...
        + '                    <h2 ng-show="picker.dateMode">{{picker.currentDate.format("dddd")}}</h2>'
        + '                    <h2 ng-show="picker.timeMode">{{picker.params.shortTime ? picker.currentDate.format("A") : " "}}</h2>'
        + '                    <span flex></span>'
        + '                    <md-button class="md-icon-button" ng-click="picker.hide()">'
        ...
        + '                <div layout="column">'
        + '                    <div class="dtp-actual-month">{{picker.currentDate.format("MMM") | uppercase}} {{picker.currentDate.format("DD") | uppercase}}</div>'
        + '                </div>'
        + '                <div layout="row">'
        + '                    <md-button class="md-icon-button" ng-click="picker.incrementYear(-1)" ng-if="picker.isPreviousYearVisible()">'
        + '                        <md-icon md-svg-icon="navigation:ic_arrow_back_24px" aria-label="Year back"></md-icon>'
        + '                    </md-button>'
        + '                    <div class="dtp-actual-year" flex>{{picker.currentDate.format("YYYY")}}</div>'
        + '                    <md-button class="md-icon-button" ng-click="picker.incrementYear(1)" ng-if="picker.isNextYearVisible()">'
        + '                        <md-icon md-svg-icon="navigation:ic_arrow_forward_24px" aria-label="Year forward"></md-icon>'
        + '                    </md-button>'
        + '                </div>'
        + '            </div>'//start time
